...
const makeRobotChoiceCard = (bot) => {
  return `
        ...
        <img id="img-two" src='${bot.imgAddress}' alt='${bot.name}'/>
        <h3>${bot.name}</h3>
        <h4>Health: ${bot.health}</h4>
        <p>Attack 1: ${bot.attacks[0].damage} damage</p>
        <p>Attack 2: ${bot.attacks[1].damage} damage</p>
        <button class="bot-btn" onclick="chooseBot(${bot.id})">Add to Gameplay</button>
        </div>
    `;
};

const makeRobotPlayerCard = (bot) => {
  let cardClass = playerDuo.includes(bot) ? "bot-card outline color-change" : "bot-card outline";
  // Add "color-change" class to the playerDuo bots if they need to change color
  ...
  return `
        ...
        <p>Attack 1: ${bot.attacks[0].damage} damage</p>
        <p>Attack 2: ${bot.attacks[1].damage} damage</p>
        <button class="bot-btn" onclick="putBotBack(${bot.id})">Remove from Gameplay</button>
        </div>
    `;
};
...
const duel = () => {
  ...
  setTimeout(() => {
    axios.post("/api/duel", { compDuo, playerDuo }).then(({ data }) => {
      ...
      if (data === "EXISTENTIAL VICTORIOUSNESS!") {
        resultsText.classList.add("color-change");
      } else if (data === "BABBLING FOOL!") {
        resultsText.classList.add("shake");
      }
...
const getAllChars = () => {
  const isVisible = allBotsDiv.style.display === "flex";

  if (isVisible) {
    allBotsDiv.style.display = "none";
    seeAllBtn.textContent = "Villainous View";
  } else {
    ...
    allBotsDiv.style.display = "flex";
    seeAllBtn.textContent = "Hide Villainous View";
  }
};
...
seeAllBtn.addEventListener("click", getAllChars);
